...
function onScroll({ scroll }) {
    ...
    if (scrollFraction > 0.80) {
        ...
        // Close sub-panels if scroll goes back
        const menSub = document.getElementById('men-subnav-panel');
        const womenSub = document.getElementById('women-subnav-panel');
        const menCh = document.getElementById('menChevron');
        const womenCh = document.getElementById('womenChevron');
        if (menSub) menSub.classList.remove('open');
        if (womenSub) womenSub.classList.remove('open');
        if (menCh) { menCh.classList.remove('open'); menCh.setAttribute('aria-expanded', 'false'); }
        if (womenCh) { womenCh.classList.remove('open'); womenCh.setAttribute('aria-expanded', 'false'); }
    }
